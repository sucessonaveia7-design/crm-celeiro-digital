...
import {
  ...
  List, AlignLeft, Type, Hash, Calendar, ToggleLeft, CheckCircle2,
} from 'lucide-react'
...
const STORAGE_KEY = 'configuracoes_campos_personalizados'
...
const EMPTY_FORM = { nome: '', tipo: 'Lista' as FieldType, opcoes: '', status: 'Ativo' as FieldStatus }
...
export default function CamposPersonalizadosSettings() {
  ...
  function openEdit(campo: Campo) {
    setForm({ nome: campo.nome, tipo: campo.tipo, opcoes: campo.opcoes?.join(', ') || '', status: campo.status })
  ...
  function handleSave() {
    ...
    if (editingId) {
      setCampos(prev => prev.map(c =>
        c.id === editingId ? { ...c, nome: form.nome.trim(), tipo: form.tipo, opcoes, status: form.status } : c
      ))
    } else {
      setCampos(prev => [...prev, {
          ...
          <div className="mb-8 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <p className="text-[11px] font-[700] text-[#D4AF37] uppercase tracking-widest mb-0.5">
                    Configurações
                  </p>
                  <h1 className="text-[24px] font-[800] text-[#0F172A] dark:text-white tracking-tight leading-none">
                    Campos Personalizados
                  </h1>
                </div>
              </div>
              <p className="text-[14px] text-[#64748B] dark:text-[#94A3B8] ml-[52px]">
                Crie informações extras para membros e visitantes.
              </p>
            </div>
          ...
          <div className="bg-white dark:bg-[#0F172A] rounded-[24px] overflow-hidden
            ...
            <div className="flex items-center justify-between px-6 py-4
              ...
              <h2 className="text-[15px] font-[700] text-[#0F172A] dark:text-white">
                Lista de campos
                <span className="text-[#94A3B8] font-[500] text-[13px] ml-1.5">({filtered.length})</span>
              </h2>
              <input
                value={search}
                  ...
                  <tbody>
                    {filtered.map((campo, idx) => (
                      <tr key={campo.id}
                        ...
                        <td className="px-6 py-4">
                          <button
                            onClick={() => toggleStatus(campo.id)}
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full
                                        text-[11px] font-[700] border transition-all cursor-pointer
                              ${campo.status === 'Ativo'
                                ...
                                : 'text-[#94A3B8] bg-[#F1F5F9] dark:bg-[#1E293B] border-[rgba(15,23,42,0.08)] dark:border-[rgba(255,255,255,0.08)] hover:bg-[#E2E8F0] dark:hover:bg-[rgba(255,255,255,0.08)]'}`}
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {(['Ativo', 'Inativo'] as FieldStatus[]).map(s => (
                    <button
                      key={s}
                      onClick={() => setForm(f => ({ ...f, status: s }))}
                      className={`flex-1 py-2.5 rounded-[12px] text-[13px] font-[600] border transition-all
                        ${form.status === s
                          ? s === 'Ativo'
                            ...
                            : 'bg-[#F1F5F9] dark:bg-[#1E293B] border-[rgba(15,23,42,0.12)] dark:border-[rgba(255,255,255,0.12)] text-[#64748B] dark:text-[#94A3B8]'
                          : 'bg-[#F8FAFC] dark:bg-[rgba(255,255,255,0.03)] border-[rgba(15,23,42,0.06)] dark:border-[rgba(255,255,255,0.06)] text-[#94A3B8] hover:border-[rgba(15,23,42,0.12)]'}`}
                    >
                      {s}
                    </button>
                  ))}
